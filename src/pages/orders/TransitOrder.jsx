...
import { get_transit_order } from './../../features/order/orderSlice';
const TransitOrder = () => {
    const { transitOrder, isLoading } = useSelector((state) => state.order);
    ...
    const handleModal = (orderNumber) => {
        const order = transitOrder.transitOrders.find(
    ...
    const content = transitOrder?.transitOrders?.map((order, i) => (
        <tr key={i}>
            ...
            <td className="py-2 px-2">
                <span
                    className={`p-2 rounded-md ${order.claim === "Yes"
        ...
        <div className="rounded-md lg:w-[90%] mx-auto p-2">
            ...
            <div className="relative overflow-x-auto bg-white p-2 border rounded border-gray-200">
                <h1 className="text-teal-500 text-2xl font-semibold text-center">
                    Transit Orders ({showOrderItems(transitOrder?.totalTransitItem)})
                </h1>
                <div className="p-2 bg-white rounded-md shadow-sm mt-1 relative overflow-x-auto ">
                    <table className="text-sm text-left font-poppin text-black w-full">
                        ...
                        <tbody className="[&>:nth-child(even)]:bg-gray-100">
                            {transitOrder?.transitOrders?.length ? (
                                content
                            ) : (
                                <tr>
                ...
                {transitOrder?.totalTransitItem > perPage && (
                    <div className="mt-3 flex justify-end mx-3">
                        <Pagination
                            pageNumber={currentPage}
                            setPageNumber={setCurrentPage}
                            totalItem={transitOrder?.totalTransitItem}
                            perPage={perPage}
                            showItem={showItem}
