...
const Sidebar = ({ showSidebar, setShowSidebar }) => {
  ...
  const { allOrder, totalDF, totalUnSettled, totalReturn, totalTransit } = useSelector(
    (state) => state.filter
  );
  console.log(totalDF, totalUnSettled, totalReturn, totalTransit);
  ...
  return (
    <div>
      ...
      {/* side bar */}
      <div
        className={`w-[260px] fixed bg-white z-50 top-0 h-screen shadow-[0_0_15px_0_rgb(34_41_47_/_5%)] transition-all ${showSidebar ? "left-0" : "-left-[260px] lg:left-0"
          }`}
      >
        {/* shop logo */}
            ...
            {allNav?.map((nav) => (
              <li key={nav.id}>
                <Link
                  ...
                  to={nav.path}
                  className={`cursor-pointer ${pathname === nav.path
                ...
                >
                  ...
                  <span
                    className={`transition-transform duration-300 ${expandedMenu === nav.id ? "rotate-180" : ""
                      }`}
                  >
                ...
                {nav.children && (
                  <ul
                    className={`pl-2 overflow-hidden transition-max-height duration-300 ease-in-out ${expandedMenu === nav.id ? "max-h-[1000px]" : "max-h-0"
                      }`}
                  >
